 ...
 * between the two is comparing from memory instead of from the screen.
 */
    ...
    <div className="flex items-stretch gap-2">
      {[spec.left, spec.right].map((side, i) => (
        <figure key={i} className="flex flex-1 flex-col gap-1.5">
          ...
          {!compact && (
            <figcaption className="text-center text-[0.75rem] font-medium text-muted-foreground">
              {side.label}
            </figcaption>
          )}
        </figure>
        ...
        <li key={i} className="relative flex flex-1 flex-col gap-1.5">
          ...
          <span
            className={cn(
              ...
              "bg-foreground/80 text-[0.625rem] font-semibold text-background",
            )}
          >
            {i + 1}
          </span>
          {!compact && (
            <p className="text-center text-[0.75rem] text-muted-foreground">{step.caption}</p>
          )}
        </li>
      ))}
    </ol>
        ...
        <div className="mt-3 flex flex-wrap justify-center gap-1.5">
          {hints.map((hint) => (
            <motion.span
              ...
              className="rounded-full bg-foreground/[0.055] px-2 py-0.5 text-[0.6875rem] text-foreground/70"
            >
              {hint}
            </motion.span>
          ))}
